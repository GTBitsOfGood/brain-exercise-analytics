...
export const POST = APIWrapper({
  ...
  handler: async (req) => {
    const {email, questionsAttempted, questionsCorrect, timePerQuestion} = await req.json();
    ...
    const data = await modifyTrivia(user._id!, questionsAttempted, questionsCorrect, timePerQuestion);
